...
class BitArrayDataset(Dataset):
    ...
    #     # anchor_tensor = np.reshape(anchor_tensor, (self.num_slices, self.num_slices, self.num_slices))
    #     # anchor_tensor = anchor_tensor.transpose(2, 1, 0)
    #     num_points = self.num_slices ** 3
    #     point_cloud = np.zeros((num_points, 3), dtype=np.float32)  # Initialize with (0,0,0)

    #     # Find positions where bitarray is 1
    #     occupied_indices = np.where(anchor_tensor == 1)[0]
    
    #     # Update those positions to (1,1,1)
    #     point_cloud[occupied_indices] = [1, 1, 1]

    #     # Transpose to get (3, num_points)
    #     xyz_channels = point_cloud.T  # Shape: (3, num_points)
        
    #     # current_size = xyz_channels.shape[1]

    #     # if current_size >= 2048:
    #     #     return xyz_channels[:, :2048]  # Trim if already at or above target size
    ...
    #     return (xyz_channels, anchor_label, anchor_class_name), self.num_slices
